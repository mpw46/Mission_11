...
function CategoryFilter ({selectedCategories, setSelectedCategories}: {selectedCategories: (string[]), setSelectedCategories: (categories: string[]) => void;})
{
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const response = await fetch('https://mission13-markus-backend-bdc2ceathzeyebdv.francecentral-01.azurewebsites.net/Book/GetCategories');
        ...
        <>
            <div className="category-filter">
                ...
                <div className="list-group">
                    {categories.map((c) => (
                        <div key={c} className={`list-group-item category-item ${selectedCategories.includes(c) ? 'active' : ''}`}>
