...
const AdminPage = () => {
    ...
    return (
        <div>
            ...
            {showForm && (
                <NewBookForm
                onSuccess={() => {setShowForm(false); fetchBooks(pageSize, pageNum, sortOrder, []).then((data) => setBooks(data.books));}}
            ...
            {editingBook && (
                <EditBookForm book={editingBook} onSuccess={() => {
                    setEditingBook(null);
                    fetchBooks(pageSize, pageNum, sortOrder, []).then((data) => setBooks(data.books))
                }}
                    ...
                    {books.map((book) => (
                        <tr key={book.bookID}>
                            ...
                            <td>
                                ...
                                <button className="btn btn-danger btn-sm w-100" onClick={() => handleDelete(book.bookID)}>Delete</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
